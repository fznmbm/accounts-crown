...
export default function Remittances() {
  ...
  const confirmSave = () => {
    ...
    const updatedInvoices = invoices.map((inv) => {
      ...
      return {
        ...
        status: Math.abs(match.amount - inv.total) < 0.02 ? "paid" : "partial",
        remittanceId: rem.id,
      };
    });
  ...
  const del = (id) => {
    ...
    if (affectedInvoices.length > 0) {
      setInvoices(
        invoices.map((inv) =>
          inv.remittanceId === id
            ? { ...inv, status: "unpaid", paidAmount: 0, remittanceId: null }
            : inv,
        ),
      );
    }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800">
              <div>
                <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">
                  PDF parsed — Payment #{preview.paymentNumber}
                </p>
                <p className="text-xs text-blue-600 dark:text-blue-400 mt-0.5">
                  {preview.items.length} invoices · {fmt(preview.total)} · due{" "}
                  {fmtD(preview.paymentDate)}
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <button className="btn-primary text-xs" onClick={confirmSave}>
                  ✓ Save &amp; reconcile invoices
                </button>
              </div>
            </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {sorted.map((r) => (
                  <tr
                    ...
                    onClick={() => setDetail(r)}
                  >
                    <td className="td font-mono font-semibold text-gray-900 dark:text-gray-100">
                      #{r.paymentNumber}
                    </td>
      ...
      {detail && (
        <Modal
          title={`Remittance — Payment #${detail.paymentNumber}`}
          onClose={() => setDetail(null)}
        >
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="metric">
              <p className="muted mb-1">Payment date</p>
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                {fmtD(detail.paymentDate)}
              </p>
            </div>
            <div className="metric">
              <p className="muted mb-1">Total paid</p>
